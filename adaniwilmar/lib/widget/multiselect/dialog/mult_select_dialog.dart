...
class _MultiSelectDialogState<T> extends State<MultiSelectDialog<T>> {
  ...
  void initState() {
    ...
    if(widget.items.length == count && count!=0){
      isSelect = true;
    }
    // print("count====${count}");
    // print("seleted length====${_selectedValues.length}");
    // print("seleted length====${widget.items.length}");
  ...
  Widget _buildListItem(MultiSelectItem<T> item) {
    return Theme(
      ...
      child: widget.singleSelection != null && widget.singleSelection!
          ? ListTile(
              title: InkWell(
              onTap: () {
                setState(() {
                ...
                if(widget.items.length == _selectedValues.length && _selectedValues.length!=0){
                  isSelect = true;
                }
          ...
          : CheckboxListTile(
              ...
              onChanged: (checked) {
                GMLogger.v(_selectedValues);
                GMLogger.v(widget.initialValue);
                print(item.value);
                if ((widget.initialValue.contains(item.value) &&
                    (widget.isUncheck == true))) {
                } else {
                  setState(() {
  ...
  Widget build(BuildContext context) {
    return CurveOuterBox(
        boxLRPadding: 0,
        boxTBPadding: 0,
        boxShadowColor: const Color(0x25000000),
        boxBorderColor: const Color(0xFFFFFFFF),
        boxofWidget: AlertDialog(
  ...
  Checkbox buildSelectAllCheckbox(List<MultiSelectItem<T>> items) {
    return Checkbox(
      value: isSelect,
      onChanged: (value) {
        setState(() {

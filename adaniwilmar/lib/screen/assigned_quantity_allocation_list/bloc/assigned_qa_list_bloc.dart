...
class AssignedQAListBloc
    extends Bloc<AssignedQAListEvent, AssignedQAListState> {
  AssignedQAListBloc() : super(InitialQuantityAllocationCreateUpdateState()) {
  ...
  AssignedQAListState get initialState => InitialQuantityAllocationCreateUpdateState();


  // Future<void> _getOverallData(
  //     LoadOverallData event, Emitter<AssignedQAListState> emit) async {
  //   try {
 ...
 /* Future<void> _getQuantityRequestList(LoadQuantityAllocationCreateUpdateRequest event,
      Emitter<AssignedQAListState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta meta = await ServiceRepository()
  ...
  /*Future<void> _saveQuantityRequest(
      SaveQuantityRequest event, Emitter<AssignedQAListState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getDistributorData(
      LoadSalesOrganization event, Emitter<AssignedQAListState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getDistributionChannelData(
      LoadDistributionChannel event, Emitter<AssignedQAListState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getVerticalData(
      LoadVerticalList event, Emitter<AssignedQAListState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getOilTypeData(
      OnLoadOilType event, Emitter<AssignedQAListState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getSubCategory(
      OnLoadSubCategory event, Emitter<AssignedQAListState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getMaterialDropDown(
      OnLoadMaterialDDList event, Emitter<AssignedQAListState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta meta =
  ...
  Future<void> _getZonalEmployeesList(
      LoadZonalEmployees event, Emitter<AssignedQAListState> emit) async {
  try {
      emit(ShowProgressBar());
...
Future<void> _saveQuantityRequest(
  ...
  Future<void> _getQuantityRequestList(LoadQuantityAllocationCreateUpdateRequest event,
        ...
        try {
          emit(ShowProgressBar());
          Meta meta = await ServiceRepository()
              .listQuantityAssignedAllocationItems();
          GMLogger.v(meta.statusMsg);
      GMLogger.v(meta.statusCode.toString());
      if (meta.statusCode == 200) {

...
import 'bloc/qa_list_bloc.dart';
import 'bloc/qa_list_event.dart';
import 'bloc/qa_list_state.dart';

class QAListScreen extends StatelessWidget {
  const QAListScreen({Key? key}) : super(key: key);
  static const String routeName = '/';

  static Route route() {
    return MaterialPageRoute(settings: const RouteSettings(name: routeName), builder: (_) => const QAListScreen());
  }
  ...
  QAListBloc _getInitialLoadData() {
    QAListBloc todayRateBloc = QAListBloc();
...
class _QualityAllocationState extends State<QualityAllocation> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<QAListBloc, QAListState>(
        listener: (context, state) {
          ...
          /* else if (state is OnLoadMaterial) {
            ...
            Utils().showSuccessDlg(context, "Quantity Request", "Quantity Request", successText: "Request Confirmed");
        ...
        child: Scaffold(
          primary: false,
          floatingActionButton: Constants.AUTH_ROLEID == Constants.NHMANAGER
              ? FloatingButton(
                  ...
                  navigationFunction: () async {
                    var res = await Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => QACreateUpdateScreen(qa: QAListResponseValue())),
                    );

                    if (res != null) {
                      BlocProvider.of<QAListBloc>(context).add(LoadQuantityAllocationCreateUpdateRequest());
                    }
                  },
                  buttoniconSize: 20)
              : Container(),
                          ...
                          itemBuilder: (context, i) {
                            return GestureDetector(
                              onTap: () async {
                                var res = await Navigator.push(
                                  context,
                                  MaterialPageRoute(builder: (context) => QACreateUpdateScreen(qa: listModel[i])),
                                );
                                          ...
                                          Expanded(
                                            ...
                                            child: Column(children: [
                                              Padding(
                                                ...
                                                child: Text("Oil Type", style: TextStyle(fontSize: 13, color: Constant.colorLightGray, fontWeight: Constant.fontWeight400)),
                                              ),
                                              Text((listModel[i].oilTypeName ?? ""), style: TextStyle(fontSize: 16, color: Constant.colorBlack, fontWeight: Constant.fontWeight600)),
                                            ]),
                                          ),
                                          Expanded(
                                            flex: 1,
                                            child: Column(
                                              children: [
                                                Padding(
                                                  padding: const EdgeInsets.only(top: 6, bottom: 10),
                                                  child: Text("Quantity Limit", style: TextStyle(fontSize: 13, color: Constant.colorLightGray, fontWeight: Constant.fontWeight400)),
                                                ),
                                                Text(listModel[i].quantityLimit.toString() ?? "-", style: TextStyle(fontSize: 16, color: Constant.colorBlack, fontWeight: Constant.fontWeight600)),
                                              ],
                                            ),
                                          ),
                                        ],
                                        ...
                                        children: [
                                          Expanded(
                                            ...
                                            child: Column(children: [
                                              Padding(
                                                padding: const EdgeInsets.only(top: 6, bottom: 10),
                                                child: Text("Valid From", style: TextStyle(fontSize: 13, color: Constant.colorLightGray, fontWeight: Constant.fontWeight400)),
                                              ),
                                              Text(DateTimeUtils().dateToServerToDateFormat((listModel[i].validFrom ?? ""), DateTimeUtils.ServerFormat, DateTimeUtils.DD_MM_YYYY_Format),
                                          ...
                                          Expanded(
                                            ...
                                            child: Column(
                                              children: [
                                                Padding(
                                                  padding: const EdgeInsets.only(top: 6, bottom: 10),
                                                  child: Text("Valid To", style: TextStyle(fontSize: 13, color: Constant.colorLightGray, fontWeight: Constant.fontWeight400)),
                                                ),
                                                Text(DateTimeUtils().dateToServerToDateFormat((listModel[i].validTo ?? ""), DateTimeUtils.ServerFormat, DateTimeUtils.DD_MM_YYYY_Format),
                                                    style: TextStyle(fontSize: 16, color: Constant.colorBlack, fontWeight: Constant.fontWeight600)),
                                              ],
                                            ),
                                          ),
    ...
    if (!popup) {
      ...
      final DateTime? selected = await showDatePicker(
          ...
          initialDate: _ptodatecontroller.text.toString() == ""
              ? DateTime.now()
              : DateTime.parse(DateTimeUtils().dateToServerToDateFormat(_ptodatecontroller.text.toString(), DateTimeUtils.DD_MM_YYYY_Format, DateTimeUtils.YYYY_MM_DD_Format)),
          firstDate: _ptodatecontroller.text.toString() == ""
              ? DateTime.now()
              : DateTime.parse(DateTimeUtils().dateToServerToDateFormat(_ptodatecontroller.text.toString(), DateTimeUtils.DD_MM_YYYY_Format, DateTimeUtils.YYYY_MM_DD_Format)),
          lastDate: DateTime.now());
      if (selected != null) {
        _ptodatecontroller.text = DateTimeUtils().dateToStringFormat(selected, DateTimeUtils.DD_MM_YYYY_Format);
      }
    }
  }
    ...
    if (selected != null) {
      if (type == "fromDate") {
        if (_todatecontroller.text.isEmpty) {
          dateController.text = DateTimeUtils().dateToStringFormat(selected, DateTimeUtils.DD_MM_YYYY_Format);
        } else {
      ...
      builder: (ctx) {
        return MultiSelectDialog<BdoList>(
          ...
          onConfirm: (List<BdoList> values) {
            ...
            BlocProvider.of<QAListBloc>(context)
  ...
  void submitQuantityAllocation() {
    if ((selectedSalesOrg == null) || (selectedSalesOrg!.id ?? 0) == 0) {
      ...
      Utils().showSuccessDlg(context, "Error", "Error", successText: "Please select the Materials");
      ...
      allocationCreateReq.validFrom = DateTimeUtils().convertDateType(_fromdatecontroller.text).toString();
      allocationCreateReq.validTo = DateTimeUtils().convertDateType(_todatecontroller.text).toString();
      allocationCreateReq.quantityLimit = double.parse(_quantitycontroller.text);

      BlocProvider.of<QAListBloc>(context).add(SaveQuantityRequest(allocationCreateReq: allocationCreateReq));
    }
  }

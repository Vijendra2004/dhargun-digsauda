...
class RequestQuantityCreateBloc
    extends Bloc<RequestQuantityCreateEvent, RequestQuantityCreateState> {
  RequestQuantityCreateBloc() : super(InitialQuantityAllocationCreateUpdateState()) {
  ...
  RequestQuantityCreateState get initialState => InitialQuantityAllocationCreateUpdateState();
 ...
 /* Future<void> _getQuantityRequestList(LoadQuantityAllocationCreateUpdateRequest event,
  ...
  Future<void> _getDistributorData(
      LoadSalesOrganization event, Emitter<RequestQuantityCreateState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getDistributionChannelData(
      LoadDistributionChannel event, Emitter<RequestQuantityCreateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getVerticalData(
      LoadVerticalList event, Emitter<RequestQuantityCreateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getOilTypeData(
      OnLoadOilType event, Emitter<RequestQuantityCreateState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getSubCategory(
      OnLoadSubCategory event, Emitter<RequestQuantityCreateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getMaterialDropDown(
      OnLoadMaterialDDList event, Emitter<RequestQuantityCreateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta meta =
  ...
  Future<void> _getZonalEmployeesList(
      LoadZonalEmployees event, Emitter<RequestQuantityCreateState> emit) async {
  try {
      emit(ShowProgressBar());
  ...
  Future<void> _saveQuantityRequest(
      SaveQuantityRequest event, Emitter<RequestQuantityCreateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta savedSauda = await ServiceRepository().saveRequestQuantityCreate(event.requestQuantityCreateReq);
      GMLogger.v(savedSauda.statusCode.toString());
      GMLogger.v(savedSauda.statusMsg);
      if (savedSauda.statusCode == 200) {
        emit(HideProgressBar());
  ...
  _getQAItem(LoadQAItemFetch event, Emitter<RequestQuantityCreateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta meta = await ServiceRepository()

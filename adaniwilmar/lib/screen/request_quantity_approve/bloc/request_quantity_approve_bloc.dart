...
import 'package:adaniwilmar/screen/request_quantity_approve/bloc/request_quantity_approve_event.dart';
import 'package:adaniwilmar/screen/request_quantity_approve/bloc/request_quantity_approve_state.dart';
...
class RequestedQuantityApproveBloc extends Bloc<RequestedQuantityApproveEvent, RequestedQuantityApproveState> {
  RequestedQuantityApproveBloc() : super(InitialQuantityAllocationCreateUpdateState()) {
    // on<LoadOverallData>((event, emit) => _getOverallData(event, emit));
    on<LoadQAItemFetch>((event, emit) => _getQAItem(event, emit));
    on<SaveQuantityRequest>((event, emit) => _saveQuantityRequest(event, emit));
    on<LoadSalesOrganization>((event, emit) => _getDistributorData(event, emit));
  ...
  RequestedQuantityApproveState get initialState => InitialQuantityAllocationCreateUpdateState();

  Future<void> _getDistributorData(LoadSalesOrganization event, Emitter<RequestedQuantityApproveState> emit) async {
    try {
  ...
  Future<void> _getDistributionChannelData(LoadDistributionChannel event, Emitter<RequestedQuantityApproveState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getVerticalData(LoadVerticalList event, Emitter<RequestedQuantityApproveState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg = await ServiceRepository().getVertical(event.distributionId, isFromQuantityAllocationCreateUpdate: true);
      GMLogger.v("Dealer" + salesOrg.statusMsg);
      GMLogger.v("Dealer" + salesOrg.statusCode.toString());
  ...
  Future<void> _getOilTypeData(OnLoadOilType event, Emitter<RequestedQuantityApproveState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg = await ServiceRepository().getOilTypeList(Constants.AUTH_USERID,
  ...
  Future<void> _getSubCategory(OnLoadSubCategory event, Emitter<RequestedQuantityApproveState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg = await ServiceRepository().getSubCategory();
      GMLogger.v("Dealer" + salesOrg.statusMsg);
      GMLogger.v("Dealer" + salesOrg.statusCode.toString());
      List<Vertical> verticals = [];
      if (salesOrg.statusCode == 200) {
  ...
  Future<void> _getMaterialDropDown(OnLoadMaterialDDList event, Emitter<RequestedQuantityApproveState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta meta = await ServiceRepository().getMaterialDropDownList(event.oilTypeId ?? "");
      GMLogger.v("list" + meta.statusMsg);
      GMLogger.v("list" + meta.statusCode.toString());
      List<BdoList> materialList = [];
      if (meta.statusCode == 200) {
        jsonDecode(meta.statusMsg)['response'].forEach((f) => materialList.add(BdoList.fromJson(f)));
      }
      if (meta.statusCode == 200) {
        emit(HideProgressBar());
  ...
  Future<void> _getZonalEmployeesList(LoadZonalEmployees event, Emitter<RequestedQuantityApproveState> emit) async {
    try {
      emit(ShowProgressBar());
      ...
      if (metaZonalEmpList.statusCode == 200) {
        jsonDecode(metaZonalEmpList.statusMsg)['response'].forEach((f) => zEmpList.add(ZonalEmployeeList.fromJson(f)));
      }
      if (metaZonalEmpList.statusCode == 200) {
        emit(HideProgressBar());
  ...
  Future<void> _saveQuantityRequest(SaveQuantityRequest event, Emitter<RequestedQuantityApproveState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta savedSauda = await ServiceRepository().saveRequestQuantityStatusUpdate(event.requestQuantityStatusUpdateReq);
  ...
  _getQAItem(LoadQAItemFetch event, Emitter<RequestedQuantityApproveState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta meta = await ServiceRepository().listFetchAssignQAReqURL(event.id);

import 'package:adaniwilmar/screen/sauda_modifications/saudaDetails/SaudaModDetailScreen.dart';
import 'package:adaniwilmar/screen/sauda_modifications/saudaList/bloc/bloc.dart';
...
import 'bloc/sauda_modification_list_bloc.dart';
import 'bloc/sauda_modification_list_event.dart';
import 'bloc/sauda_modification_list_state.dart';

class SaudaListScreen extends StatelessWidget {
  const SaudaListScreen({Key? key}) : super(key: key);
  static const String routeName = '/';

  static Route route() {
    return MaterialPageRoute(
        settings: const RouteSettings(name: routeName),
        builder: (_) => const SaudaListScreen());
  }

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => SaudaModificationListBloc()
        ..add(LoadSaudaModificationList(
      ...
      child: const SaudaModListScreen(),
    );
  }
}

class SaudaModListScreen extends StatefulWidget {
  const SaudaModListScreen({Key? key}) : super(key: key);

  @override
  State<SaudaModListScreen> createState() => _SaudaModListScreenState();
}

class _SaudaModListScreenState extends State<SaudaModListScreen>
  ...
  List<DealerGroup> dealers = [];
  List<DealerGroup> approvedealers = [];
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<SaudaModificationListBloc, SaudaModificationListState>(
        listener: (context, state) {
          if (state is OnSaudaModListSuccess) {
            dealers.clear();
            approvedealers.clear();
            setState(() {
              approvedealers = state.approvedBdoDealerList!;
              dealers = state.pendingList!;
              approvedCount = approvedealers.length;
              pendingCount = dealers.length;
            });
          }
          if (state is ShowModListProgress) {
            setState(() {
              isLoading = true;
            });
          }
          if (state is HideModListProgress) {
            setState(() {
              isLoading = false;
            });
          }
          ...
          appBar: CustomAppBar(
              ...
              listOfActions: Row(
                children: [
                  IconButton(
                    onPressed: () {
                      showCustomFilterDialog(context, "Filter Date",
                          "Filter Date", dialogActionButtonFilter());
                    },
                    icon: SizedBox(
                      width: 30.0,
                      height: 30.0,
            ...
            children: [
              ...
              Container(
                  margin: const EdgeInsets.only(
                      top: 60, left: 8, right: 8, bottom: 10),
                  child: CurveBorderBox(
                      boxLRPadding: 0, boxofWidget: tabmenuOfExt())),
              if (isLoading)
                Container(
              ...
              child: FloatingButton(
                  ...
                  navigationFunction: () async {
                    final result = await Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (context) =>
                              const SaudaModificationCreation()),
                    );
                    if (result == true) {
                      BlocProvider.of<SaudaModificationListBloc>(context).add(
                          LoadSaudaModificationList(
                              DateTimeUtils().dateToStringFormat(
                                  DateTime.now().add(const Duration(
                                      days: Constants.REPORT_START_DAY)),
                                  DateTimeUtils.YYYY_MM_DD_Format),
                              DateTimeUtils().dateToStringFormat(DateTime.now(),
                                  DateTimeUtils.YYYY_MM_DD_Format)));
                    }
                  },
                  buttoniconSize: 20)),
        ...
        children: [
          ...
          SizedBox(
            ...
            child: CommonButton(
              ...
              buttonFunction: () {
                BlocProvider.of<SaudaModificationListBloc>(ct).add(
                    LoadSaudaModificationList(
  ...
  _selectToDate(BuildContext context) async {
    final DateTime? selected = await showDatePicker(
      ...
      initialDate: _todatecontroller.text.toString() == ""
          ? DateTime.now().add(const Duration(
              days: Constants.REPORT_MAX_DAY + Constants.REPORT_START_DAY))
          : DateTime.parse(DateTimeUtils().dateToServerToDateFormat(
              _todatecontroller.text.toString(),
              DateTimeUtils.DD_MM_YYYY_Format,
              DateTimeUtils.YYYY_MM_DD_Format)),
          ...
          Expanded(
            child: Padding(
              ...
              child: TabBarView(
                controller: tabController,
                children: [
                  _pendingSaudaModificationListScreen(),
                  _approvedSaudaModificationListScreen(),
                ],
              ),
            ),
          ),
  ...
  Widget _pendingSaudaModificationListScreen() {
    return MediaQuery.removePadding(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final dealerName = dealer.dealerName;
          final items = dealer.items as List;
                                ...
                                Row(
                                  ...
                                  children: [
                                    ...
                                    Expanded(
                                      flex: 2,
                                      child: Text(
                                        items[idx].approvalRejectedByName ?? '',
  ...
  Widget _approvedSaudaModificationListScreen() {
    return MediaQuery.removePadding(
      context: context,
      removeTop: true,
      child: ListView.builder(
        itemCount: approvedealers.length,
        itemBuilder: (context, index) {
          final dealer = approvedealers[index];
          final dealerName = dealer.dealerName;
          final items = dealer.items as List;
                          ...
                          return InkWell(
                            onTap: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => SaudaModDetailScreen(
                                        id: items[idx].id)),
                              );
                            },
                            child: Padding(

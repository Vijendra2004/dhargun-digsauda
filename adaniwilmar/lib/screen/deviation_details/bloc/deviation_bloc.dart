...
import 'package:adaniwilmar/screen/deviation_details/bloc/bloc.dart';
...
class DeviationBloc extends Bloc<DeviationEvent, DeviationState> {
  DeviationBloc() : super(InitialDeviationState()) {
    on<LoadDeviationScreen>((event, emit) => _getDeviationData(event, emit));
  }
  DeviationState get initialState => InitialDeviationState();

  Future<void> _getDeviationData(
      LoadDeviationScreen event, Emitter<DeviationState> emit) async {
    try {

...
class NewSpecialRateBloc
    extends Bloc<NewSpecialRateEvent, NewSpecialRateState> {
  NewSpecialRateBloc() : super(InitialNewSpecialRateState()) {
    ...
    on<LoadNewSpecialRateScreen>(
        (event, emit) => _getDistributorList(event, emit));
    on<LoadSKUDetails>((event, emit) => _getSKUData(event, emit));
    on<SaveSpecialRate>((event, emit) => _saveSpecialRate(event, emit));
    on<ChangeRate>((event, emit) => _changeRate(event, emit));
  }
  NewSpecialRateState get initialState => InitialNewSpecialRateState();

  Future<void> _getDistributorList(
      LoadNewSpecialRateScreen event, Emitter<NewSpecialRateState> emit) async {
    try {
  ...
  Future<void> _getSalesOrganization(
      LoadSalesOrganization event, Emitter<NewSpecialRateState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getDistributorData(
      LoadDealerSaudaDetail event, Emitter<NewSpecialRateState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getSKUData(
      LoadSKUDetails event, Emitter<NewSpecialRateState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getDistributionChannelData(
      LoadDistributionChannel event, Emitter<NewSpecialRateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getVerticalData(
      LoadVerticalList event, Emitter<NewSpecialRateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getOilTypeData(
      LoadOilType event, Emitter<NewSpecialRateState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getIncoTermsData(
      LoadIncoTerms event, Emitter<NewSpecialRateState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getPlantData(
      LoadPlant event, Emitter<NewSpecialRateState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  // @override
  // Future<void> _getOverallData(LoadOverallData event,Emitter<NewSpecialRateState> emit)async {
  //   try {
  //     emit(ShowProgressBar());
  ...
  //   }
  // }
  Future<void> _saveSpecialRate(
      SaveSpecialRate event, Emitter<NewSpecialRateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta savedSauda =
          await ServiceRepository().saveSpecialRate(event.request);
      GMLogger.v(savedSauda.statusCode.toString());
      GMLogger.v(savedSauda.statusMsg);
      if (savedSauda.statusCode == 200) {
        emit(HideProgressBar());
        emit(OnSaveSpecialRate(
  ...
  Future<void> _changeRate(
      ChangeRate event, Emitter<NewSpecialRateState> emit) async {

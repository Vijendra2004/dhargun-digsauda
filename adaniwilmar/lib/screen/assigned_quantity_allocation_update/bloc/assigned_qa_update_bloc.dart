...
import 'package:adaniwilmar/screen/assigned_quantity_allocation_update/bloc/asigned_qa_update_event.dart';
...
class AssignedQAUpdateBloc
    extends Bloc<AssignedQAUpdateEvent, AssignedQAUpdateState> {
  AssignedQAUpdateBloc() : super(InitialQuantityAllocationCreateUpdateState()) {
  ...
  AssignedQAUpdateState get initialState => InitialQuantityAllocationCreateUpdateState();
 ...
 /* Future<void> _getQuantityRequestList(LoadQuantityAllocationCreateUpdateRequest event,
  ...
  Future<void> _getDistributorData(
      LoadSalesOrganization event, Emitter<AssignedQAUpdateState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getDistributionChannelData(
      LoadDistributionChannel event, Emitter<AssignedQAUpdateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getVerticalData(
      LoadVerticalList event, Emitter<AssignedQAUpdateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getOilTypeData(
      OnLoadOilType event, Emitter<AssignedQAUpdateState> emit) async {
    try {
      emit(ShowProgressBar());
  ...
  Future<void> _getSubCategory(
      OnLoadSubCategory event, Emitter<AssignedQAUpdateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta salesOrg =
  ...
  Future<void> _getMaterialDropDown(
      OnLoadMaterialDDList event, Emitter<AssignedQAUpdateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta meta =
  ...
  Future<void> _getZonalEmployeesList(
      LoadZonalEmployees event, Emitter<AssignedQAUpdateState> emit) async {
  try {
      emit(ShowProgressBar());
  ...
  Future<void> _saveQuantityRequest(
      SaveQuantityRequest event, Emitter<AssignedQAUpdateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta savedSauda = await ServiceRepository().saveAssignedQuantityAllocationUpdateRequest(event.allocationCreateReq);
      GMLogger.v(savedSauda.statusCode.toString());
      GMLogger.v(savedSauda.statusMsg);
      if (savedSauda.statusCode == 200) {
        emit(HideProgressBar());
  ...
  _getQAItem(LoadQAItemFetch event, Emitter<AssignedQAUpdateState> emit) async {
    try {
      emit(ShowProgressBar());
      Meta meta = await ServiceRepository()
          .listFetchAssignQAReqURL(event.id);
      GMLogger.v(meta.statusMsg);
      GMLogger.v(meta.statusCode.toString());
      if (meta.statusCode == 200) {
